...
template <typename T>
int	predict_sum(const std::vector<T> &vec, int sum) {
	size_t n = vec.size(), left = 0, right = vec.size() - 1, min_diff = INT_MAX, min_sum = 0;
...
int main() {
	...
	std::cout << predict_sum(vec, 54) << std::endl;
